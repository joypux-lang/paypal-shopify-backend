// server.js (updated defaults for your store & 2025-10 API)
...
const {
  ...
  SHOPIFY_ADMIN_TOKEN,
  SHOPIFY_API_VERSION = "2025-10"   // defaulted to 2025-10 per your setting
} = process.env;
...
app.post("/api/shopify/order-from-paypal", async (req, res) => {
  try {
    ...
    const draftInput = {
      ...
      lineItems: b.line_items.map(li => ({ variantId: toVariantGID(li.variant_id), quantity: parseInt(li.quantity,10), price: li.price ? li.price.toString() : null })),
      shippingLine: (b.shipping_price !== "" && b.shipping_price != null) ? { title: (b.shipping_label || "Shipping"), price: b.shipping_price.toString() } : null,
